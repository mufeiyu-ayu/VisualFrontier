import type { CommonResultType, CreateAxiosOptionsType, RequestConfigType, RequestOptionsType, UploadFileParamsType } from '@ayu-mu/model'
  ...
  private setupInterceptors() {
    ...
    try {
      ...
      this.axiosInstance.interceptors.request.use((config: InternalAxiosRequestConfig & RequestConfigType) => {
        // 合并请求选项
        const { isShowLoading, loadingMessageText, joinParamsToUrl, headers } = {
          ...requestOptions,
      ...
      this.axiosInstance.interceptors.response.use((response: AxiosResponse<CommonResultType<unknown>>) => {
        ...
        if (response.data?.code !== ResponseCodeEnum.SUCCESS) {
          ...
          && (errorMessageText
            ...
            : ElMessage.error({ message: response.data?.message || response.data?.message, zIndex: 99999 }))
        }
